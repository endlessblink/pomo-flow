...
import { Eye, EyeOff, CheckCircle, Circle, Minimize2, Maximize2, AlignCenter, ListTodo, Calendar as CalendarIcon, Play, Check } from 'lucide-vue-next'
...
onMounted(() => {
  ...
  console.log('🔧 BoardView: Adding test done tasks for Done column verification...')
  setTimeout(() => {
    if (taskStore.tasks.filter(t => t.status === 'done').length === 0) {
      // Only add test done tasks if none exist
      ...
      taskStore.createTaskWithUndo({
        title: '✅ Completed task 2 - Test Done Column',
        description: 'Another test task to verify Done column shows multiple items',
        status: 'done',
        projectId: '1'
      })

      console.log('🔧 BoardView: Added 2 test done tasks')
    }
  }, 1000)
})
...
/* Adjust for 2-column layout (when done column is hidden) */
