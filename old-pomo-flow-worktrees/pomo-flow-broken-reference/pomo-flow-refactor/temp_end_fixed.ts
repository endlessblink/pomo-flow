...
export const useTaskStore = defineStore('tasks', () => {
  ...
  const hideDoneTasks = ref(true) // Global setting to hide done tasks across all views (enabled by default)
  ...
  const loadFromDatabase = async () => {
    ...
    // Load hide done tasks setting (defaults to true for better productivity)
    ...
    // If no saved setting exists, keep the default value of true
      ...
      startTaskNowWithUndo: async (taskId: string) => {
        try {
          ...
          if (actions && typeof actions.startTaskNow === 'function') {
            ...
            return localStartTaskNow(taskId)
