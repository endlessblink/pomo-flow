...
const handleSectionResizeStart = ({ sectionId, event }: any) => {
  // Resize started - no action needed
}
...
const handleSectionResize = ({ sectionId, event }: any) => {
  ...
  if (width && height) {
    // Dimensions are being tracked during resize
  }
}
...
const handleSectionResizeEnd = ({ sectionId, event }: any) => {
  ...
  console.log('📏 [CanvasView] Extracted dimensions:', {
    width,
    height,
    source: event?.params?.width ? 'event.params.width' : event?.width ? 'event.width' : 'NONE'
  })

  if (width && height) {
    const section = canvasStore.sections.find(s => s.id === sectionId)
    if (section) {
      console.log('📐 [CanvasView] Current section state:', {
      ...
      console.log('✅ [CanvasView] Validated dimensions (will persist):', {
        validatedWidth,
        validatedHeight,
        wasWidthClamped: validatedWidth !== Math.abs(width),
        wasHeightClamped: validatedHeight !== Math.abs(height),
        clampedBy: {
          width: validatedWidth !== Math.abs(width) ? (validatedWidth === 200 ? 'MIN (200)' : 'MAX (2000)') : 'NONE',
          height: validatedHeight !== Math.abs(height) ? (validatedHeight === 80 ? 'MIN (80)' : 'MAX (2000)') : 'NONE'
        }
      })
      ...
      console.log('💾 [CanvasView] Dimensions persisted to store')
    } else {
      console.error('❌ [CanvasView] Section not found:', sectionId)
    }
  } else {
...
const distributeHorizontal = () => {
  ...
  sorted.forEach((node, index) => {
    ...
    taskStore.updateTaskWithUndo(node.id, {
      canvasPosition: { x: newX, y: node.position.y }
    })
  })
...
const distributeVertical = () => {
  ...
  sorted.forEach((node, index) => {
    ...
    taskStore.updateTaskWithUndo(node.id, {
      canvasPosition: { x: node.position.x, y: newY }
    })
  })
...
const arrangeInGrid = () => {
  ...
  selectedNodes.forEach((node, index) => {
    ...
    const newX = startX + (col * SPACING_X)
    const newY = startY + (row * SPACING_Y)
