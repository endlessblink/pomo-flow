...
const syncNodes = () => {
  ...
  // Canvas loading verification - log detailed information about task filtering
  const verifyCanvasData = () => {
    const allTasks = taskStore.tasks
    const filteredTasks = taskStore.filteredTasks
    const canvasTasks = allTasks.filter(task => !task.isInInbox && task.canvasPosition)
    const filteredCanvasTasks = filteredTasks.filter(task => !task.isInInbox && task.canvasPosition)

    console.log(`🔧 Canvas Verification Report:`)
    console.log(`   Total tasks in store: ${allTasks.length}`)
    console.log(`   Filtered tasks (active project): ${filteredTasks.length}`)
    console.log(`   Tasks with canvas position: ${canvasTasks.length}`)
    console.log(`   Filtered tasks with canvas position: ${filteredCanvasTasks.length}`)

    // Log details about tasks that should appear but don't
    if (canvasTasks.length > filteredCanvasTasks.length) {
      const hiddenByProject = canvasTasks.filter(task =>
        !filteredTasks.some(ft => ft.id === task.id)
      )
      console.log(`   Tasks hidden by project filter: ${hiddenByProject.length}`)
      hiddenByProject.forEach(task => {
        console.log(`     - "${task.title}" (project: ${task.projectId})`)
      })
    }

    // Log tasks that are in inbox but have canvas positions (data inconsistency)
    const inconsistentTasks = allTasks.filter(task =>
      task.isInInbox && task.canvasPosition
    )
    if (inconsistentTasks.length > 0) {
      console.log(`   ❌ Tasks with inconsistent data (in inbox but have canvas position): ${inconsistentTasks.length}`)
      inconsistentTasks.forEach(task => {
        console.log(`     - "${task.title}" (${task.canvasPosition?.x}, ${task.canvasPosition?.y})`)
      })
    }

    return filteredCanvasTasks
  }

  // Add task nodes AFTER (so they render on top) - only if section is NOT collapsed
  const expectedCanvasTasks = verifyCanvasData()
  ...
  for (const node of selectedNodes) {
    ...
    if (node.id.startsWith('section-')) {
      ...
      try {
        ...
        console.log('✅ After deleteTask - task count updated')
      } catch (error) {
      ...
      try {
        ...
        console.log('✅ After updateTaskWithUndo - task position updated')
      } catch (error) {
...
const handleQuickTaskCreate = (title: string, description: string) => {
  ...
  const newTask = taskStore.createTaskWithUndo({
    ...
    priority: 'medium',
    status: 'planned',
    projectId: taskStore.activeProjectId || '1' // Use active project filter or default to My Tasks
  })
