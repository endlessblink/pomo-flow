...
const handleDragStart = (event: DragEvent, task: Task) => {
  ...
  if (event.target instanceof HTMLElement) {
    ...
    if (tableRow) {
      tableRow.classList.add('dragging')
    }
  }
}
...
const handleDragEnd = () => {
  ...
  const draggingRows = document.querySelectorAll('.table-row.dragging')
...
:global(body.dragging-active) .table-row {
  opacity: 0.8;
  transition: opacity var(--duration-fast);
}

/* Keep dragged row fully visible */
:global(body.dragging-active) .table-row.dragging {
  opacity: 1 !important;
}
