    ...
    <main class="main-content" :class="{ 'sidebar-hidden': !uiStore.mainSidebarVisible }">
      ...
      <div class="content-header">
        <div class="view-tabs">
          ...
          <router-link to="/weekend" class="view-tab" active-class="active">
            This Weekend
            <span v-if="weekendTaskCount > 0" class="tab-badge">{{ weekendTaskCount }}</span>
          </router-link>
...
import { NConfigProvider, NMessageProvider, NGlobalStyle, darkTheme, lightTheme } from 'naive-ui'
...
// Unified undo/redo composable for task operations
const { createTaskWithUndo, deleteTaskWithUndo, updateTaskWithUndo, undo, redo } = useUnifiedUndoRedo()
...
// Naive UI theme that matches our theme store
const naiveUITheme = computed(() => isDarkMode.value ? darkTheme : lightTheme)
...
// Store-Database Comparison Tool
const compareStoreWithDatabase = async () => {
  console.log('🔍 App.vue: Comparing store state with IndexedDB database...')

  try {
    // Import the database composable
    const { useDatabase } = await import('@/composables/useDatabase')
    const db = useDatabase()

    // Load raw data from IndexedDB
    const dbProjects = await db.load('projects') || []
    const dbTasks = await db.load('tasks') || []

    // Get current store state
    const storeProjects = taskStore.projects
    const storeTasks = taskStore.tasks

    console.log('📊 Store vs Database Comparison:', {
      database: {
        projectsCount: dbProjects.length,
        tasksCount: dbTasks.length,
        projects: dbProjects.map(p => ({ id: p.id, name: p.name, parentId: p.parentId })),
        tasks: dbTasks.slice(0, 3).map(t => ({ id: t.id, title: t.title, projectId: t.projectId }))
      },
      store: {
        projectsCount: storeProjects.length,
        tasksCount: storeTasks.length,
        projects: storeProjects.map(p => ({ id: p.id, name: p.name, parentId: p.parentId })),
        tasks: storeTasks.slice(0, 3).map(t => ({ id: t.id, title: t.title, projectId: t.projectId }))
      },
      discrepancies: {
        projectsMismatch: dbProjects.length !== storeProjects.length,
        tasksMismatch: dbTasks.length !== storeTasks.length
      }
    })

    // Check for specific missing projects
    const missingInStore = dbProjects.filter(dbProj =>
      !storeProjects.find(storeProj => storeProj.id === dbProj.id)
    )

    const extraInStore = storeProjects.filter(storeProj =>
      !dbProjects.find(dbProj => dbProj.id === storeProj.id)
    )

    if (missingInStore.length > 0) {
      console.warn('⚠️ Projects in database but missing from store:', missingInStore)
    }

    if (extraInStore.length > 0) {
      console.warn('⚠️ Projects in store but not in database:', extraInStore)
    }

    return {
      database: { projects: dbProjects, tasks: dbTasks },
      store: { projects: storeProjects, tasks: storeTasks },
      missingInStore,
      extraInStore
    }

  } catch (error) {
    console.error('❌ Error comparing store with database:', error)
    return null
  }
}

// Force Store Refresh Function
const refreshStoreFromDatabase = async () => {
  console.log('🔄 App.vue: Force refreshing store from IndexedDB...')
  ...
  try {
    ...
    // Load raw data directly from IndexedDB
    const dbProjects = await db.load('projects') || []
    const dbTasks = await db.load('tasks') || []

    console.log('📊 Direct database load:', {
      projectsCount: dbProjects.length,
      tasksCount: dbTasks.length,
      projects: dbProjects.map(p => ({ id: p.id, name: p.name, parentId: p.parentId }))
    })

    // Force update store with raw database data (bypassing smart merge)
    taskStore.projects = dbProjects
    taskStore.tasks = dbTasks

    console.log('✅ Store force updated with raw database data')
    console.log('🔍 Updated store state:', {
      projectsCount: taskStore.projects.length,
      tasksCount: taskStore.tasks.length,
      rootProjects: taskStore.projects.filter(p => !p.parentId).map(p => ({ id: p.id, name: p.name }))
    })

    return true
  } catch (error) {
    console.error('❌ Error force refreshing store:', error)
    return false
  }
}
...
const rootProjects = computed(() => {
  const result = taskStore.projects.filter(p => !p.parentId)

  // DEBUG: Log rootProjects computation
  console.log('🔍 App.vue rootProjects computed:', {
    totalProjectsInStore: taskStore.projects.length,
    rootProjectsCount: result.length,
    rootProjects: result.map(p => ({
      id: p.id,
      name: p.name,
      parentId: p.parentId,
      color: p.color
    })),
    allProjects: taskStore.projects.map(p => ({
      id: p.id,
      name: p.name,
      parentId: p.parentId
    }))
...
// Use the store's unified computed getter - SINGLE SOURCE OF TRUTH
const weekendTaskCount = computed(() => {
  console.log('🗓️ Weekend Counter Debug (STORE UNIFIED):', {
    totalTasks: taskStore.tasks.length,
    totalProjects: taskStore.projects.length
  })

  // The taskStore.weekendTaskCount is now the single source of truth
  return taskStore.weekendTaskCount
})


// Uncategorized task count for Quick Sort badge - using store computed getter
const uncategorizedCount = computed(() => {
  console.log('📋 Uncategorized count (STORE UNIFIED):', taskStore.uncategorizedTaskCount)
  return taskStore.uncategorizedTaskCount
})
...
const createQuickTask = () => {
  if (newTaskTitle.value.trim()) {
    ...
    createTaskWithUndo({
      ...
      description: '',
      status: 'planned',
      projectId: taskStore.activeProjectId || '1' // Use active project or default
    })
    newTaskTitle.value = ''
  }
}
...
const handleQuickTaskCreate = (title: string, description: string) => {
  ...
  // Use the unified undo system
  const newTask = createTaskWithUndo({
    title: title,
...
const selectSmartView = (view: 'today' | 'weekend') => {
  ...
  // CRITICAL FIX: Navigate to actual weekend route
  if (view === 'weekend') {
    router.push('/weekend')
  }
}
...
onMounted(async () => {
  console.log('🚀 App.vue onMounted: Starting initialization...')
  ...
  console.log('📊 App.vue: Loading data from IndexedDB...')
  await taskStore.loadFromDatabase()
  await canvasStore.loadFromDatabase()

  // DEBUG: Log store state after database loading
  console.log('🔍 App.vue: Store state after database loading:', {
    taskStoreProjects: taskStore.projects.length,
    taskStoreTasks: taskStore.tasks.length,
    projectDetails: taskStore.projects.map(p => ({
      id: p.id,
      name: p.name,
      parentId: p.parentId,
      color: p.color,
    ...
    taskDetails: taskStore.tasks.slice(0, 5).map(t => ({
      id: t.id,
      title: t.title,
  ...
  // Compare store with database to identify discrepancies
  const comparison = await compareStoreWithDatabase()

  // If there are discrepancies, force refresh the store
  if (comparison && (comparison.missingInStore.length > 0 || comparison.extraInStore.length > 0)) {
    console.log('⚠️ Discrepancies detected, force refreshing store...')
    await refreshStoreFromDatabase()
  }
...
/* CONTROL PANEL - Transparent wrapper for separate widgets */
.control-panel {
  display: flex;
  align-items: center;
  gap: var(--space-3);  /* Minimal spacing: 12px */
  ...
  /* Re-enable pointer events for child interactions */
  pointer-events: auto;
}
...
/* TIME DISPLAY CONTAINER - Transparent, no background */
