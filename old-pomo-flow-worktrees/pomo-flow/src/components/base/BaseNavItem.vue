<template>
  <div
    ...
    :data-project-name="$attrs['data-project-name']"
    :draggable="!!projectId"
...
const handleDragEnter = (event: DragEvent) => {
  ...
  // Also handle raw HTML5 drag data for components not using useDragAndDrop
  const dragData = event.dataTransfer?.getData('application/json')
  if (dragData) {
    try {
      const parsedData = JSON.parse(dragData)
      // If this is task data and it's not already tracked by useDragAndDrop, start tracking it
      if (parsedData.type === 'task' && parsedData.taskId) {
        startDrag(parsedData)
      }
    } catch (error) {
...
const handleDrop = (event: DragEvent) => {
  ...
  if (dragData.value && isDragValid.value) {
    ...
    if (dragData.value.type === 'task') {
      // Handle both single task and batch operations from inboxes
      const taskIds = dragData.value.projectIds || [dragData.value.taskId].filter(Boolean)
      if (taskIds.length > 0) {
        taskIds.forEach(taskId => {
          taskStore.moveTaskToProject(taskId, props.projectId)
        })
...
/* Brand primary highlight when mouse enters a valid drop target */
