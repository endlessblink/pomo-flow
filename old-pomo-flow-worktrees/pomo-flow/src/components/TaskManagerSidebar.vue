...
// 🎯 FIXED: Use ONLY store's unified filtering - no local overrides!
const filteredTasks = computed(() => {
  console.log('🔍 TaskManagerSidebar: Using store unified filtering (no local filters)')
  console.log('🔍 TaskManagerSidebar: Sidebar filter state:', { activeFilter: activeFilter.value, searchQuery: searchQuery.value })

  // Update store filters based on sidebar state
  if (activeFilter.value !== 'all') {
    taskStore.setActiveSmartView(activeFilter.value === 'unscheduled' ? 'inbox' : null)
  }
  if (searchQuery.value) {
    // Store doesn't have search text filter, so we need to handle this
    // But keep it consistent with how other components would search
  }

  console.log('🔍 TaskManagerSidebar: Store filtered tasks count:', taskStore.filteredTasks.length)
