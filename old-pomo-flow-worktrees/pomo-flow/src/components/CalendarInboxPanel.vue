...
// 🎯 FIXED: Use unified filtering system
const inboxTasks = computed(() => {
  console.log('🗓️ CalendarInboxPanel: Using unified filtering for filter:', currentFilter.value)

  // Map calendar filters to unified smart views
  let smartView: string | null = null
  ...
  switch (currentFilter.value) {
    ...
    case 'unscheduled':
      smartView = 'inbox'
      additionalFilter.hasInstances = false
      break
    case 'notOnCanvas':
      additionalFilter.showInInbox = true
      additionalFilter.showPositioned = false
      break
    case 'incomplete':
      additionalFilter.includeCompleted = false
      break
    case 'allTasks':
      smartView = null
      break
  ...
  const filteredTasks = filterTasksByView(
    ...
    'calendar',
    {
      ...
      hideDoneTasks: currentFilter.value === 'incomplete' ? true : false,
      hasInstances: additionalFilter.hasInstances,
      projects: taskStore.projects
    },
    taskStore.projects
  )

  console.log('🗓️ CalendarInboxPanel: Unified filtering result:', filteredTasks.length, 'tasks')
...
const addTask = () => {
  ...
  taskStore.createTask({
    ...
    description: '',
    status: 'planned',
    projectId: taskStore.getDefaultProjectId(), // Use proper default project
    isInInbox: true
  })
...
const handleDragStart = (event: DragEvent, task: Task) => {
  ...
  // Use standardized drag data format
  const dragData = {
    type: 'task',
    taskIds: [task.id], // Array format for batch operations
...
const handleDragEnd = () => {
  ...
  // Reset any visual feedback that wasn't cleaned up by the timeout
  document.querySelectorAll('.inbox-task-card').forEach(card => {
