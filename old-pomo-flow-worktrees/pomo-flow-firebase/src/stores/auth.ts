...
export const useAuthStore = defineStore('auth', () => {
  ...
  /**
   * Load user profile from Firestore
   * GRACEFUL DEGRADATION: If Firestore fails (permissions), auth still works
   */
  async function loadUserProfile(uid: string): Promise<void> {
    try {
      ...
      if (profileSnap.exists()) {
        ...
        console.warn('⚠️ Profile document does not exist, attempting to create')
        await createUserProfile(uid)
      }
    } catch (err: any) {
      // CRITICAL: Don't block auth if Firestore fails (permissions error)
      if (err.code === 'permission-denied' || err.message?.includes('insufficient permissions')) {
        console.warn('⚠️ Firestore permissions not set up - auth will work but profile won\'t save to cloud')
        console.warn('ℹ️ To fix: Deploy firestore.rules from project root to Firebase Console')
        // Create minimal local profile so UI components work
        profile.value = {
          uid: user.value?.uid || uid,
   ...
   * GRACEFUL DEGRADATION: If Firestore fails, creates local profile
   */
  async function createUserProfile(uid: string): Promise<void> {
    ...
    try {
      ...
      // CRITICAL: Don't block auth if Firestore fails
      if (err.code === 'permission-denied' || err.message?.includes('insufficient permissions')) {
        console.warn('⚠️ Cannot create profile in Firestore (permissions) - creating local profile')
        // Create local profile anyway
        profile.value = {
          ...newProfile,
          createdAt: now,
          lastLoginAt: now
