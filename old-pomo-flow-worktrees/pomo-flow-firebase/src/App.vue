  ...
  <div class="app" :dir="direction">
    ...
    <main class="main-content" :class="{ 'sidebar-hidden': !uiStore.mainSidebarVisible }">
      ...
      <div class="header-section">
        ...
        <!-- POMODORO TIMER DISPLAY (Right side) -->
      ...
      <div class="content-header">
        <div class="view-tabs">
          ...
          <router-link to="/quick-sort" class="view-tab" active-class="active">
            <Zap :size="16" style="margin-right: 4px;" />
            Quick Sort
...
const createQuickTask = async () => {
  if (newTaskTitle.value.trim()) {
    ...
    const { createTaskWithUndo } = await import('@/composables/useUnifiedUndoRedo')
    createTaskWithUndo({
...
const confirmDeleteTask = async (task: Task) => {
  ...
  confirmAction.value = async () => {
    // Use the unified undo system
    const { deleteTaskWithUndo } = await import('@/composables/useUnifiedUndoRedo')
...
const handleRedo = async () => {
  ...
  try {
    const { redo } = await import('@/composables/useUnifiedUndoRedo')
    await redo()
...
.header-section {
  ...
  justify-content: space-between;
  align-items: center;
  gap: var(--space-6); /* Add spacing between UserProfile, title, and timer */
...
/* USER PROFILE CONTAINER - Simplified for bulletproof clicks */
...
.content-header {
  ...
  padding-bottom: 0; /* Removed to align border with sidebar at same Y-coordinate */
