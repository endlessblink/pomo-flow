<template>
  ...
  >
    ...
    <div v-if="emoji" class="project-emoji">
      {{ emoji }}
    </div>
    <div
      v-else-if="colorDot"
...
const isDragValid = computed(() => {
  ...
  if (dragData.value.type === 'project' && dragData.value.projectId) {
    ...
    // Can't drop a parent onto its own child (would create circular reference)
    const draggedProject = taskStore.getProjectById(dragData.value.projectId)
    if (draggedProject?.parentId === props.projectId) return false
