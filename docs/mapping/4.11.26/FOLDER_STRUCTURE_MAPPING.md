...
└── controls/                    # Phase 3: Extracted canvas controls (NEW)
    └── CanvasControls.vue       # Centralized zoom and viewport controls
```

### Recurring Tasks Components (src/components/recurrence/) - **NEW IN v4.11.26**
```
src/components/recurrence/
├── RecurrencePatternSelector.vue  # UI for selecting recurrence patterns
├── RecurrencePreview.vue          # Preview of recurring task instances
├── RecurrenceEditModal.vue        # Modal for editing recurrence rules
└── index.ts                       # Component exports
```

### Notifications Components (src/components/notifications/) - **NEW IN v4.11.26**
```
src/components/notifications/
├── NotificationPreferences.vue    # User notification settings panel
├── NotificationCenter.vue       # Central notification display
├── NotificationToast.vue          # Individual notification toast
├── TaskReminderModal.vue          # Modal for task reminders
└── index.ts                       # Component exports
```
...
├── notifications.ts             # **NEW** Task reminders and notifications system (v4.11.26)
...
├── canvas/                       # **NEW** Canvas-specific composables (v4.11.26)
│   ├── useCanvasControls.ts      # Phase 3: Centralized zoom and viewport controls
│   ├── useCanvasContextMenu.ts   # Phase 3: Extracted canvas context menu logic
│   └── useCanvasTaskManagement.ts # Phase 3: Canvas-specific task operations
...
├── useTaskRecurrence.ts          # **NEW** Recurring task logic and instance generation (v4.11.26)
...
├── recurrence.ts                # **NEW** Recurrence pattern and instance types (v4.11.26)
