<template>
  <div class="kanban-swimlane" :class="{ collapsed: isCollapsed }">
    ...
    <div v-if="!isCollapsed" class="table-scroll-container">
...
// Define drag event types for better type safety
interface DragChangeEvent<T = any> {
  added?: { element: T; index: number }
  removed?: { element: T; index: number }
...
const taskCache = ref<Map<string, Task[]>>(new Map())

// Helper functions to group tasks with memoization
const getTasksByStatus = (status: string) => {
  const cacheKey = `status_${status}_${props.tasks.length}_${props.currentFilter}`

  if (taskCache.value.has(cacheKey)) {
    return taskCache.value.get(cacheKey)!
  }

  const result = props.tasks.filter(task => task.status === status)
  taskCache.value.set(cacheKey, result)

  // Limit cache size to prevent memory leaks
  if (taskCache.value.size > 50) {
    const firstKey = taskCache.value.keys().next().value
    taskCache.value.delete(firstKey)
  }
    ...
    return instances.some(instance => {
      ...
      switch (dateColumn) {
        case 'inbox':
          ...
          return !task.dueDate && task.status === 'backlog'
        case 'today':
...
const getTasksByPriority = (priority: string) => {
  const cacheKey = `priority_${priority}_${props.tasks.length}_${props.currentFilter}`
...
watch(() => props.tasks, () => {
  // Clear cache when tasks change to ensure fresh calculations
...
const handleDragChange = (event: DragChangeEvent<Task>, viewType: string, columnKey: string) => {
  // Add defensive context guard to prevent null context errors
  if (!event) {
    console.warn('Drag event is null or undefined')
    return
  }
  ...
  if (event.added) {
    ...
    if (!task || !task.id) {
      console.warn('Invalid task in drag event:', task)
      return
    }
    ...
    try {
      ...
      console.error('Error handling drag change:', error)
    }
  }

  // Handle removed events if needed for future enhancements
  if (event.removed) {
    // Task was removed from this column
    // This could be used for logging or analytics
  }

  // Handle moved events within the same column
  if (event.moved) {
    // Task was reordered within the same column
    // This could be used for position persistence
  }
}
