...
.calendar-event {
  ...
  /* Container queries for responsive padding */
  container-type: inline-size;
  container-name: calendar-event;
}
...
.calendar-event .priority-stripe {
  position: absolute;
  top: 0;
  inset-inline-start: 20px; /* RTL: offset after emoji stripe (20px) */
  width: 4px;
  height: 100%;
...
.event-content {
  ...
  padding-left: 28px; /* Space for emoji (20px) + priority (4px) + spacing (4px) */
  cursor: move;
  transition: all var(--transition-fast);
  display: flex;
  flex-direction: column;
...
/* Container queries for responsive padding based on event width */
@container calendar-event (max-width: 120px) {
  .event-content {
    padding-left: 20px; /* Reduced for medium-width events */
  }
  .calendar-event .priority-stripe {
    inset-inline-start: 14px; /* Adjusted offset */
  }
}

@container calendar-event (max-width: 80px) {
  .event-content {
    padding-left: 12px; /* Minimal padding for narrow events */
  }
  .calendar-event .priority-stripe {
    inset-inline-start: 8px; /* Adjusted offset */
  }
}
...
.week-event .priority-stripe {
  position: absolute;
  top: 0;
  inset-inline-start: 20px; /* RTL: offset after emoji stripe (16px + 4px gap) */
  width: 4px;
  height: 100%;
  border-start-end-radius: var(--radius-sm); /* RTL: top-right in LTR, top-left in RTL */
  border-end-end-radius: var(--radius-sm); /* RTL: bottom-right in LTR, bottom-left in RTL */
...
.week-event .event-content {
  ...
  justify-content: center;
  padding: 0;
  padding-left: 28px; /* Space for emoji (16px) + priority (4px) + spacing (8px) */
}

/* Container queries for responsive padding in week view */
@container week-event (max-width: 100px) {
  .week-event .event-content {
    padding-left: 18px; /* Reduced for medium-width week events */
  }
  .week-event .priority-stripe {
    inset-inline-start: 12px; /* Adjusted offset */
  }
}

@container week-event (max-width: 60px) {
  .week-event .event-content {
    padding-left: 10px; /* Minimal padding for narrow week events */
  }
  .week-event .priority-stripe {
    inset-inline-start: 6px; /* Adjusted offset */
  }
}
...
.month-event {
  ...
  padding-left: 20px; /* Space for emoji (12px) + priority (3px) + spacing (5px) */
  border-radius: var(--radius-md);
  color: var(--text-primary);
  cursor: move;
  overflow: hidden;
  ...
  backdrop-filter: blur(8px);
  box-shadow: var(--shadow-sm);
  position: relative; /* For absolutely positioned stripes */
  /* Container queries for responsive padding */
  container-type: inline-size;
  container-name: month-event;
}

/* Container queries for responsive padding in month view */
@container month-event (max-width: 80px) {
  .month-event {
    padding-left: 14px; /* Reduced for medium-width month events */
  }
  .month-event .priority-stripe {
    inset-inline-start: 10px; /* Adjusted offset */
  }
}

@container month-event (max-width: 50px) {
  .month-event {
    padding-left: 8px; /* Minimal padding for narrow month events */
  }
  .month-event .priority-stripe {
    inset-inline-start: 5px; /* Adjusted offset */
  }
}
...
.month-event .priority-stripe {
  position: absolute;
  top: 0;
  inset-inline-start: 15px; /* RTL: offset after emoji indicator (12px + 3px gap) */
  width: 3px;
  height: 100%;
  border-start-end-radius: var(--radius-sm); /* RTL: top-right in LTR, top-left in RTL */
  border-end-end-radius: var(--radius-sm); /* RTL: bottom-right in LTR, bottom-left in RTL */
