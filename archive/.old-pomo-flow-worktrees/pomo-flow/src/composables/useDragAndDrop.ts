...
export interface DragData {
  type: 'task' | 'project'
  taskId?: string
  projectIds?: string[] // For batch operations from inboxes
  projectId?: string
  title: string
  source: 'kanban' | 'calendar' | 'canvas' | 'sidebar' | 'table' | 'calendar-inbox' | 'canvas-inbox'
  fromInbox?: boolean // Legacy support for inbox dragging
  payload?: any
}
...
export function useDragAndDrop() {
  ...
  const createDragData = (
    ...
    title,
    source,
    fromInbox: source === 'calendar-inbox' || source === 'canvas-inbox',
    payload
  })
