...
const syncNodes = () => {
  ...
  canvasStore.sections.forEach(section => {
    allNodes.push({
      ...
      style: {
        ...
        height: section.isCollapsed ? '80px' : `${section.position.height}px`,
        zIndex: 1,
  ...
  taskStore.filteredTasks.forEach((task, index) => {
    ...
    if (section) {
      // Keep absolute positioning - no parent relationship to prevent child movement
      // Visual containment is handled via coordinate checking (lines 636-641)
...
const handleSectionResize = ({ sectionId, event }: any) => {
  ...
  // Try to extract dimensions from various possible event structures
  const width = event?.width || event?.params?.x || event?.dimensions?.width || event?.x
  const height = event?.height || event?.params?.y || event?.dimensions?.height || event?.y

  if (width && height) {
    console.log(`📏 [CanvasView] New dimensions detected: ${width}x${height}`)
  } else {
...
const handleSectionResizeEnd = ({ sectionId, event }: any) => {
  ...
  // Extract final dimensions - try multiple possible locations
  const width = event?.width || event?.params?.x || event?.dimensions?.width || event?.x
  const height = event?.height || event?.params?.y || event?.dimensions?.height || event?.y

  console.log('🔍 [CanvasView] Dimension extraction attempt:', {
    hasWidth: !!width,
    hasHeight: !!height,
    width,
    height,
    willPersist: !!(width && height)
  })

  if (width && height) {
    const section = canvasStore.sections.find(s => s.id === sectionId)
    if (section) {
      const validatedWidth = Math.max(200, Math.min(1200, width))
      const validatedHeight = Math.max(150, Math.min(800, height))

      console.log(`💾 [CanvasView] Persisting validated dimensions: ${validatedWidth}x${validatedHeight}`)
...
// Arrange handlers (Obsidian-style)
const arrangeInRow = () => {
  const selectedNodes = nodes.value.filter(n =>
    canvasStore.selectedNodeIds.includes(n.id) && n.type === 'taskNode'
  )
  ...
  const SPACING = 240 // Task node width (200) + gap (40)

  // Find starting x position
  const startX = sorted[0].position.x

  // Arrange nodes in a horizontal row
  sorted.forEach((node, index) => {
    taskStore.updateTaskWithUndo(node.id, {
...
const arrangeInColumn = () => {
  ...
  // Standard spacing between nodes
  const SPACING = 120 // Task node height (80) + gap (40)
...
const arrangeInGrid = () => {
  ...
  // Calculate optimal grid dimensions (closest to square)
  const count = selectedNodes.length
  const cols = Math.ceil(Math.sqrt(count))
  const rows = Math.ceil(count / cols)

  // Standard spacing
  const SPACING_X = 240 // Task node width (200) + gap (40)
  const SPACING_Y = 120 // Task node height (80) + gap (40)

  // Calculate center position of all nodes
  const avgX = selectedNodes.reduce((sum, n) => sum + n.position.x, 0) / selectedNodes.length
  const avgY = selectedNodes.reduce((sum, n) => sum + n.position.y, 0) / selectedNodes.length

  // Calculate grid start position (centered around average)
