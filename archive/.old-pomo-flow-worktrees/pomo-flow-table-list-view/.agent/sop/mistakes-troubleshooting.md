...
### 1. Section Resize System Failure (RESOLVED - October 2025)
**Problem:** NodeResizer not working - vertical resize blocked above thresholds, extremely laggy performance (10-15fps).

**Mistakes Made:**
1. **Parent-Child Approach** - Attempted to add `parentNode` relationships to make tasks move with sections
   - **Result:** All sections coupled together, couldn't move anything ❌
   - **Action:** Immediate revert with `git checkout`

2. **CSS GPU Acceleration** - Added `backface-visibility`, `perspective`, `contain` properties
   - **Result:** Made performance worse, didn't solve anything ❌
   - **Action:** Immediate revert with `git checkout`

3. **Root Cause Misidentification** - Tried performance optimizations before identifying real issue

**Root Causes Discovered via Research:**
1. **Inline Style Conflict**: Section nodes had `width`/`height` in `style` object
   - NodeResizer updates node **PROPERTIES**, not inline CSS styles
   - Inline styles have higher specificity and override programmatic changes
   - Created update loop preventing resize

2. **CSS Transition Conflict**: Inline `transition: 'height 0.3s'` restarted on every resize event
   - Resize fires 60+ events per second
   - Transition restarting caused severe lag

3. **Validation Mismatch**: CanvasView validation (150-800px) didn't match NodeResizer props (80-2000px)

**Correct Solution:**
```javascript
// WRONG (Blocks NodeResizer):
allNodes.push({
  style: {
    width: `${section.position.width}px`,    // ❌ Inline style blocks resize
    height: `${height}px`,                   // ❌ Blocks vertical resize
    transition: 'height 0.3s ease-in-out'   // ❌ Causes lag
  }
})

// CORRECT (Works with NodeResizer):
allNodes.push({
  width: section.position.width,             // ✅ Node properties
  height: section.isCollapsed ? 80 : section.position.height,
  style: {
    zIndex: 1                                // ✅ Only styling props in style
  }
})

// Match validation to NodeResizer props:
...
**Files Modified:**
- `src/views/CanvasView.vue` (lines 579-586, 1311-1312)
- `src/components/canvas/SectionNodeSimple.vue` (lines 44-46)

**Prevention Rules:**
1. ✅ **Research Before Implementing** - Use Perplexity to understand framework behavior
2. ✅ **Revert Fast** - If approach breaks existing functionality, immediate `git checkout`
3. ✅ **Evidence-Based Fixes** - Fix root causes identified by research, not symptoms
4. ✅ **Validation Alignment** - Always match validation constraints to component props
5. ❌ **Never use inline `width`/`height` in `style` with NodeResizer**
6. ❌ **Never add CSS transitions to frequently-updated properties**

**Results:**
- ✅ Smooth resize performance restored
- ✅ Vertical and horizontal resize works without limitations
- ✅ All resize handles functional

**Commits:**
- `83712ae` - fix(canvas): resolve section resize bugs and reduce console noise
- `0131c25` - fix(canvas): sync CanvasView dimension validation with NodeResizer limits

### 2. Canvas Scrolling Issues (RESOLVED)
...
### 5. Favicon Not Updating in Browser Tab (RESOLVED - October 16, 2025)
**Problem:** Browser tab favicon wasn't changing during different timer states (idle → work → break).

**Initial Misunderstanding:**
- Assumed favicon functionality was broken because the browser tab icon wasn't visually changing
- Thought the VueUse `useFavicon` composable wasn't working
- Believed the flame emoji functionality wasn't implemented correctly

**Root Cause Discovery:**
- The favicon system was actually working correctly
- The issue was that I was looking for a specific meditation emoji (🧎) instead of testing if any favicon changes were happening
- User reference image showed a person in meditative pose, but I didn't initially understand it was about the **browser tab favicon**, not just in-app icons

**Debugging Process:**
1. Added debug console logging to `useFavicon.ts` to track state changes
2. Used Playwright MCP to test actual browser tab favicon changes
3. Verified timer state transitions with console logs
4. Captured screenshots showing browser tab favicon actually changing
5. Confirmed all three favicon states were working correctly

**What Was Actually Working All Along:**
- 🍅 **Idle/Paused State**: Tomato emoji favicon when timer is idle or paused
- 🔥 **Active Work State**: Fire emoji favicon when timer is actively running (not on break)
- 🧎 **Break State**: Meditation emoji favicon during break sessions

**Files Updated:**
- `src/composables/useFavicon.ts` - Added meditation emoji icon and break state logic
- `src/stores/timer.ts` - Updated session type icon computation
- `src/App.vue` - Updated in-app break display icon
- `src/views/FocusView.vue` - Updated focus view break icon
...
// Favicon state logic
if (isActive && !isBreak) {
  faviconHref.value = fireIcon      // 🔥 during work
}
else if (isActive && isBreak) {
  faviconHref.value = meditationIcon  // 🧎 during break
}
else {
  faviconHref.value = tomatoIcon      // 🍅 when idle
}
```

**Validation Results:**
- ✅ Playwright MCP captured screenshots confirming actual browser tab favicon changes
- ✅ Console logs showed proper state transitions
- ✅ All timer states tested and verified working
- ✅ Both in-app icons and browser tab favicon synchronized

**Lesson Learned:**
1. **Trust but Verify**: The existing implementation was correct, but needed visual confirmation
2. **Browser Tab vs In-App Icons**: User was specifically asking about browser tab favicon, not just app UI icons
3. **Debug Before Assuming**: Added logging to understand what was actually happening vs. what I thought should happen
4. **Visual Confirmation Required**: Playwright MCP screenshots proved the functionality was working

**Prevention:**
- Test with visual confirmation before claiming features are broken
- Understand user requirements completely (browser tab favicon vs in-app icons)
- Use debug logging to track actual vs. expected behavior
- Verify both app UI and browser tab changes when relevant
