...
### **4. Section Resize System - RESOLVED (October 2025)**

**Problem**: NodeResizer was not working properly - vertical resize was blocked above certain thresholds, and resize performance was extremely laggy (10-15fps).

**Root Causes**:
1. **Inline Style Conflict**: Section nodes had `width` and `height` in the `style` object, blocking NodeResizer
2. **CSS Transition Conflict**: Inline `transition: 'height 0.3s'` restarted on every resize event (60+ times/second)
3. **Validation Mismatch**: CanvasView validation constraints didn't match NodeResizer props

**Technical Details**:
- Vue Flow's NodeResizer updates node WIDTH/HEIGHT **PROPERTIES**, not inline CSS styles
- Inline styles have higher CSS specificity and override programmatic property changes
- This created an update loop where NodeResizer couldn't modify dimensions

**Solution**:
```javascript
// BEFORE (Broken):
style: {
  width: `${section.position.width}px`,    // ❌ Blocked NodeResizer
  height: `${height}px`,                   // ❌ Blocked vertical resize
  transition: 'height 0.3s ease-in-out'   // ❌ Caused lag
}

// AFTER (Fixed):
width: section.position.width,             // ✅ Node properties (not style)
height: section.isCollapsed ? 80 : section.position.height,
style: {
  zIndex: 1                                // ✅ Clean style object
}
```

**Validation Alignment**:
- Updated CanvasView validation to match NodeResizer props
- Width: 200-2000px (was 200-1200px)
- Height: 80-2000px (was 150-800px)
...
- ✅ Vertical resize (up/down) now works without threshold limitations
- ✅ Horizontal resize works properly
