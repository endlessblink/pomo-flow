    ...
    return {
      ...
      deleteSectionWithUndo: async (sectionId: string) => {
        try {
          const { useUnifiedUndoRedo } = await import('@/composables/useUnifiedUndoRedo')
          const actions = useUnifiedUndoRedo()
          return actions.deleteSection(sectionId)
        } catch (error) {
          console.warn('Undo/Redo system not available, using direct updates:', error)
      ...
      // Smart section creator wrappers with undo/redo
      createPrioritySectionWithUndo: async (priority: 'high' | 'medium' | 'low', position: { x: number; y: number }) => {
        const section = createPrioritySection(priority, position)
        try {
          const { useUnifiedUndoRedo } = await import('@/composables/useUnifiedUndoRedo')
          const actions = useUnifiedUndoRedo()
          return actions.createSection(section)
        } catch (error) {
          console.warn('Undo/Redo system not available, using direct creation:', error)
          return createSection(section)
        }
      },
      createStatusSectionWithUndo: async (status: 'planned' | 'in_progress' | 'done', position: { x: number; y: number }) => {
        const section = createStatusSection(status, position)
        try {
          const { useUnifiedUndoRedo } = await import('@/composables/useUnifiedUndoRedo')
          const actions = useUnifiedUndoRedo()
          return actions.createSection(section)
      ...
      createProjectSectionWithUndo: async (projectId: string, position: { x: number; y: number }) => {
        const section = createProjectSection(projectId, position)
        try {
          const { useUnifiedUndoRedo } = await import('@/composables/useUnifiedUndoRedo')
          const actions = useUnifiedUndoRedo()
          return actions.createSection(section)
