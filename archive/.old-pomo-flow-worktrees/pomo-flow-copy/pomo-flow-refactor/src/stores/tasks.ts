...
export const useTaskStore = defineStore('tasks', () => {
  ...
  const migrateProjectColors = () => {
    console.log('🔧 DEBUG: Starting project color migration...')
    let migratedCount = 0

    projects.value.forEach(project => {
      if (!project.colorType) {
        // Enhanced emoji detection - catches common emojis including 🍅 📝 ⚡ 🎯 💡
        if (project.color && typeof project.color === 'string' &&
            (/[\p{Emoji_Presentation}\p{Emoji}\u200D]/u.test(project.color) ||
             /[🍅📝⚡🎯💡🚀⭐🔥💎🌟⚡📊🎨🔧🛠️💼📱🌍🎯🏆🎪🎭🎪🎬🎮🕹️🎯🏀⚽⚾🎾🏈🎱🏓🏸🥅🎳🏏🥋🥊🥇🥈🥉🏅🎖️🏆🥇🥈🥉🍕🍔🍟🌭🍿🥨🥙🍗🍖🌮🌯🥗🥘🍝🍜🍛🍣🍤🍙🍚🍘🍥🥣🍲🍛🥧🍨🍦🥧🍧🎂🍰🎂🍰🍪🍫🍬🍭🍮🍯🍞🥐🥖🥨🧀🥚🍳🥞🥓🥔🍟🥙🌶️🫑🥒🥬🥦🧄🧅🍄🥜🌰🍞🥐🥖🥨🧀🥚🍳🥞🥓🥔🍟🥙🌶️🫑🥒🥬🥦🧄🧅🍄🥜🌰🍞🥐🥖🥨🧀🥚🍳🥞🥓🥔🍟🥙🌶️🫑🥒🥬🥦🧄🧅🍄🥜🌰]/u.test(project.color))) {
          console.log(`🔧 DEBUG: Migrating project "${project.name}" to emoji type:`, project.color)
    ...
    console.log(`🔧 DEBUG: Project color migration completed. Migrated ${migratedCount} projects to emoji type.`)
  }
  ...
  const forceMigrateEmojiProjects = () => {
    console.log('🔧 DEBUG: Force migrating emoji projects...')
    let migratedCount = 0

    projects.value.forEach(project => {
      // Check if color contains common emojis (including 🍅)
      if (project.color && typeof project.color === 'string' &&
          /[🍅📝⚡🎯💡🚀⭐🔥💎🌟]/u.test(project.color)) {
        console.log(`🔧 DEBUG: Force migrating project "${project.name}" (${project.id}):`, {
          oldColor: project.color,
          oldColorType: project.colorType,
          oldEmoji: project.emoji
        })
    ...
    console.log(`🔧 DEBUG: Force migration completed. Migrated ${migratedCount} projects.`)
  ...
  return {
    ...
    // Migration functions
    forceMigrateEmojiProjects,

    // Debug helper (accessible via window for testing)
    debugProjects: () => {
      ...
      projects.value.forEach(project => {
        console.log(`  "${project.name}" (${project.id}):`, {
          ...
          emoji: project.emoji,
          isEmoji: project.colorType === 'emoji' && project.emoji
        })
      })
