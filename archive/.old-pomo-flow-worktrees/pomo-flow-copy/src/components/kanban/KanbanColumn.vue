...
// Handle drag-drop changes with defensive context guard
const handleDragChange = (event: DragChangeEvent<Task>) => {
  // Add defensive context guard to prevent null context errors
  if (!event) {
    console.warn('Drag event is null or undefined in KanbanColumn')
    return
  }
  ...
  if (event.added) {
    ...
    if (!task || !task.id) {
      console.warn('Invalid task in drag event in KanbanColumn:', task)
      return
    }
    ...
    try {
      ...
      console.error('Error handling drag change in KanbanColumn:', error)
    }
  }
