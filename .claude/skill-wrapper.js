...
class SkillWrapper {
  ...
  async runSkillCommand(skillName, args) {
    return new Promise((resolve, reject) => {
      ...
      const skillCommands = {
        'qa-testing': 'npm run test',
        'ops-port-manager': 'echo "Port management skill executed"',
        'dev-vue': 'echo "Vue development skill executed"'
      };
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!skillName) {
    ...
    console.log('Available skills: qa-testing, ops-port-manager, dev-vue');
    process.exit(1);
  }
